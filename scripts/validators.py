...
def check_tension_arc(text, target_tension=None):
    ...
    short_sent_ratio = sum(1 for l in sent_lens if l <= 8) / max(len(sent_lens), 1)

    # 2. 标点密度（感叹号+问号密度=情绪强度）
    exclam_quest = len(re.findall(r'[！？!?]', text))
    punct_density = exclam_quest / max(total_chars / 100, 1)

    # 3. 情绪词密度（高压情绪词）
    high_tension_words = [
        r'杀', r'死', r'血', r'恨', r'怒', r'爆', r'炸', r'毁', r'灭',
        r'背叛', r'真相', r'危机', r'绝境', r'最后', r'拼了', r'决一',
        r'跪下', r'求饶', r'碾压', r'秒杀', r'反转', r'打脸',
        r'来不及', r'怎么办', r'不可能', r'怎么会',
    ]
    tension_word_count = 0
    for p in high_tension_words:
        tension_word_count += len(re.findall(p, text))
    tension_word_density = tension_word_count / max(total_chars / 100, 1)

    # 4. 反转词密度（"竟然"/"原来"/"没想到"等）
    reversal_words = [r'竟然', r'原来', r'没想到', r'突然', r'居然', r'不料', r'谁知']
    reversal_count = sum(len(re.findall(w, text)) for w in reversal_words)
    reversal_density = reversal_count / max(total_chars / 100, 1)

    # 5. 对话占比（高对话=低叙事张力，低对话+高动作=高张力）
    dialogue_chars = 0
    for m in re.finditer(r'[^\s△#\-【\d\n]{1,10}[：:].+', text):
    ...
    # 6. 动作密度（△行占比）
    action_lines = len(re.findall(r'^△', text, re.MULTILINE))
    total_lines = max(text.count('\n'), 1)
    action_ratio = action_lines / total_lines

    # 综合张力评分 (0-10)
    # 每个因子映射到0-1，加权求和
    short_sent_score = min(short_sent_ratio * 5, 1.0)  # 0->0, 0.5->1.0
    punct_score = min(punct_density / 3, 1.0)  # 3+/100chars -> 1.0
    tension_word_score = min(tension_word_density / 2, 1.0)  # 2+/100chars -> 1.0
    reversal_score = min(reversal_density * 2, 1.0)  # 0.5+/100chars -> 1.0
    dialogue_penalty = max(1.0 - dialogue_ratio, 0)  # less dialogue = more tension
    action_score = min(action_ratio * 2, 1.0)  # more action = more tension

    raw_tension = (
        short_sent_score * 2.0 +
        punct_score * 2.0 +
        tension_word_score * 2.5 +
        reversal_score * 1.5 +
        dialogue_penalty * 1.0 +
        action_score * 1.0
    )  # max = 10.0

    actual_tension = round(min(raw_tension, 10.0), 1)

    # 对比目标张力
    if target_tension is not None:
        deviation = abs(actual_tension - target_tension)
        if deviation <= 1.0:
            passed = True
            status = "匹配"
        elif deviation <= 2.0:
            ...
            passed = False
            status = "严重偏离"
        detail = f"目标{target_tension} 实际{actual_tension} 偏差{deviation:.1f} [{status}]"
    else:
        passed = actual_tension >= 2.0  # minimum tension threshold
        detail = f"实际张力{actual_tension}/10"
        deviation = None
    ...
    return {
        ...
        "deviation": deviation,
        "components": {
            "short_sentence_ratio": round(short_sent_ratio, 3),
            "punct_density": round(punct_density, 1),
            "tension_word_density": round(tension_word_density, 1),
            "reversal_density": round(reversal_density, 1),

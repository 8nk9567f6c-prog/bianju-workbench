# 编剧工作台 — 四 Agent 调度框架 v6.0

> **v6.0**: 从 16 Agent + 35 命令精简为 4 Agent 框架。审核+对标全部内置为自审系统，调度智能体成为唯一入口。
...
| **Agent 3** — 剧本 | `agent_content_expansion.md` | 制片编剧，逐集写 v5.0 格式剧本 | **四维护剧本自审**（写作红线 15 项/平台红线 12 项/格式规范 15 项/节奏情绪 12 项）+ MCP 16 工具自动验证 + 定向修改 | S+ ≥ 85/100 + 自审通过 |

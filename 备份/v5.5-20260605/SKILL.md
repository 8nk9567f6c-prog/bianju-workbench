...
description: 短剧编剧工作台 v5.5——职业下沉市场短剧创作 Agent 系统。三智能体协作流水线(选题调研→大纲搭建→内容扩充)，100集×5w+字标准化产出，SS/S+ 爆款算法，双模情绪引擎(情绪银行+情绪弹簧)，35斜杠命令，5 Agent 并行审核，8基因对标+风格指纹，MCP 自动验证(16工具80+项)。融合救猫咪+麦基+悉德菲尔德+红果算法+drama-creator方法论+好莱坞评估标准。
version: "5.5.0"
author: "编剧工作台"
tags: [screenwriting, short-drama, content-creation, chinese, script-writing, agent-pipeline]
allowed-tools:
  - Read
  - Write
  - Edit
...
# 编剧工作台 v5.5 — 短剧创作 Skill

## 能力概述

为下沉市场（30岁以上用户，80%消费时长）创作标准化 100 集 × 5w+ 字短剧剧本。以 SS/S+ 爆款算法（10维100分制，S+≥85分）为最高优先级。v5.5 新增三智能体协作流水线(选题调研→大纲搭建→内容扩充)，一键串联全流程。

### v5.5 核心升级 vs 竞品

| 能力维度 | 编剧工作台 v5.5 | drama-creator v2.2 | @gonzih screenwriting |
|----------|----------------|-------------------|----------------------|
| 智能体流水线 | ★三智能体协作(选题→大纲→内容) | — | — |
...
```
编剧工作台/
├── CLAUDE.md              ← 主 Agent 指令集（~235行，35斜杠命令 + 5 Agent 审核 + 三智能体流水线）
